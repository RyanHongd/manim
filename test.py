...
class Add31And54Dot(Scene):
    def construct(self):
        ...
        # 如果有进位，显示进位
        if carry:
            carry_text = Text("1", font="Noto Sans CJK", font_size=40, color=WHITE).move_to(RIGHT * (digits // 2 + 1) + UP * 1.5)
            self.play(Write(carry_text))
            self.wait(1)
        ...
        tens_digits_text = Text(str(tens_digits * 10 + carry * 10), font="Noto Sans CJK", font_size=40).move_to(RIGHT * 6 + DOWN * 1)
        equal_text = Text("=", font="Noto Sans CJK", font_size=40).move_to(RIGHT * 4 + DOWN * 2.5)
        sum_text = Text(str(sum_result), font="Noto Sans CJK", font_size=40).move_to(RIGHT * 4 + DOWN * 3)

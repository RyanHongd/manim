...
class Subtract_Function(Scene):
    ...
    def Add_Function(self):
        ...
        # 創建文字
        s1 = f"小名有24塊錢, 媽媽再給他13塊錢, 現在共有多少錢?"
        s2 = f"首先我們有{n1}塊錢"
        s3 = f"媽媽再給我們{n2}塊"
        s4 = f"我們可以把十位數跟個位數分開"
        ...
        self.play(sum_text.animate.move_to(RIGHT * 3 + DOWN * 2))

        # 結果
        self.play(Write(ans))
        self.wait(1)
        self.play(FadeOut(digits_text), FadeOut(add_text), FadeOut(tens_digits_text), FadeOut(equal_text), FadeOut(sum_text))
    ...
    def Subtract_Function(self):
        ...
        # 創建文字
        s1 = f"小名有37塊錢, 給了弟弟14塊錢, 現在還剩多少錢?"
        s2 = f"首先我們有{n1}塊錢"
        s3 = f"給了弟弟{n2}塊"
        s4 = f"我們可以把十位數跟個位數分開"
        s5 = f"{n1}可以被分成{n1 // 10}個10和{n1 % 10}個1"
        s6 = f"{n2}可以被分成{n2 // 10}個10和{n2 % 10}個1"
        s7 = f"把所有的1扣掉, 再把所有的10扣掉"
        s8 = f"最後再把十位數和個位數減掉"
        s9 = f"因此我們最後剩下{n1 - n2}塊錢"
